...
async fn echo_function_invoke_sync() -> anyhow::Result<()> {
    ...
    let resp = invoke(&daemon, name, serde_json::json!({"hello":"world"}), Some(("RequestResponse","Tail"))).await?;
    ...
    assert!(resp.function_error.is_none(), "unexpected function error: {:?}", resp.function_error);
...
async fn echo_function_invoke_with_version() -> anyhow::Result<()> {
    ...
    // Invoke with version
    let resp = invoke(&daemon, name, serde_json::json!({"test":"version"}), Some(("RequestResponse","Tail"))).await?;
...
async fn python_echo_function() -> anyhow::Result<()> {
    ...
    let resp = invoke(&daemon, name, serde_json::json!({"python":"test"}), Some(("RequestResponse","Tail"))).await?;
    assert!(resp.function_error.is_none());
    let payload = as_json(&resp.payload)?;
    assert_eq!(payload["ok"], true);

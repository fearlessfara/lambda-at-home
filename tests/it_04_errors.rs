...
"#;
    ...
    let resp = invoke(&daemon, name, serde_json::json!({"test":"error"}), Some(("RequestResponse","Tail"))).await?;
    
    // Should return 200 with error header
    assert_eq!(resp.status_code, 200);
    assert!(resp.function_error.is_some());
    assert_eq!(resp.function_error, Some(lambda_models::FunctionError::Unhandled));
...
"#;
    ...
    let resp = invoke(&daemon, name, serde_json::json!({"test":"timeout"}), Some(("RequestResponse","Tail"))).await?;
...
async fn invalid_function_name() -> anyhow::Result<()> {
    ...
    let result = client.create_function(lambda_models::CreateFunctionRequest {
    ...
    assert!(error.to_string().contains("InvalidParameterValueException") || 

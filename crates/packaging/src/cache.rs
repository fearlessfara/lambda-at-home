...
impl PackagingCache {
    ...
    pub fn store_zip_file(&self, zip_info: &ZipInfo) -> Result<PathBuf, LambdaError> {
        ...
        // Write ZIP file
        fs::write(&zip_path, &zip_info.zip_data)
            .map_err(|e| LambdaError::InternalError { reason: e.to_string() })?;
    ...
    pub fn cleanup_old_cache(&mut self, max_age_days: u64) -> Result<usize, LambdaError> {
        ...
        if zip_dir.exists() {
            for entry in fs::read_dir(&zip_dir)
                .map_err(|e| LambdaError::InternalError { reason: e.to_string() })? {
                let entry = entry.map_err(|e| LambdaError::InternalError { reason: e.to_string() })?;
    ...
    fn load_cache(&mut self) -> Result<(), LambdaError> {
        ...
        if image_cache_file.exists() {
            let cache_data = fs::read_to_string(&image_cache_file)
                .map_err(|e| LambdaError::InternalError { reason: e.to_string() })?;
            
            self.image_cache = serde_json::from_str(&cache_data)
                .map_err(|e| LambdaError::InternalError { reason: e.to_string() })?;
        }
        ...
        if zip_cache_file.exists() {
            let cache_data = fs::read_to_string(&zip_cache_file)
                .map_err(|e| LambdaError::InternalError { reason: e.to_string() })?;
            
            self.zip_cache = serde_json::from_str(&cache_data)
                .map_err(|e| LambdaError::InternalError { reason: e.to_string() })?;
        }
    ...
    pub fn save_cache(&self) -> Result<(), LambdaError> {
        ...
        let image_cache_file = self.cache_dir.join("image_cache.json");
        let cache_data = serde_json::to_string_pretty(&self.image_cache)
            .map_err(|e| LambdaError::InternalError { reason: e.to_string() })?;
        fs::write(&image_cache_file, cache_data)
            .map_err(|e| LambdaError::InternalError { reason: e.to_string() })?;
        
        // Save ZIP cache to disk
        let zip_cache_file = self.cache_dir.join("zip_cache.json");
        let cache_data = serde_json::to_string_pretty(&self.zip_cache)
            .map_err(|e| LambdaError::InternalError { reason: e.to_string() })?;
        fs::write(&zip_cache_file, cache_data)
            .map_err(|e| LambdaError::InternalError { reason: e.to_string() })?;

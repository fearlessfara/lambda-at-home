...
impl ZipHandler {
    ...
    pub async fn process_zip(&self, zip_data: &[u8]) -> Result<ZipInfo, LambdaError> {
        ...
        let mut archive = ZipArchive::new(std::io::Cursor::new(zip_data)).map_err(|e| {
            LambdaError::InvalidZipFile {
                reason: e.to_string(),
            }
        })?;
        ...
        for i in 0..archive.len() {
            let file = archive
                .by_index(i)
                .map_err(|e| LambdaError::InvalidZipFile {
                    reason: e.to_string(),
                })?;
        ...
        info!(
            "Processed ZIP with {} files, total size: {} bytes, SHA256: {}",
            files.len(),
            total_size,
            sha256
        );
    ...
    pub async fn extract_to_directory(
        ...
        for i in 0..archive.len() {
            ...
            if let Some(parent) = file_path.parent() {
                std::fs::create_dir_all(parent).map_err(|e| LambdaError::InvalidZipFile {
                    reason: e.to_string(),
                })?;
            }
            ...
            if file.name().ends_with('/') {
                std::fs::create_dir_all(&file_path).map_err(|e| LambdaError::InvalidZipFile {
                    reason: e.to_string(),
                })?;
                continue;
            }
            ...
            std::fs::write(&file_path, file_data).map_err(|e| LambdaError::InvalidZipFile {
                reason: e.to_string(),
            })?;

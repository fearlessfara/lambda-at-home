...
async fn healthz_ok() {
    let app = test_router();
    let res = app
        .oneshot(Request::get("/runtime/healthz").body(Body::empty()).unwrap())
        .await
        .unwrap();
...
async fn next_blocks_then_returns_when_pushed() {
    ...
    let fut: JoinHandle<_> = tokio::spawn(async move {
        let uri = format!("/2018-06-01/runtime/invocation/next?fn=hello&rt=nodejs18.x&ver=LATEST&eh={}", correct_env_hash);
        app_clone
...
async fn runtime_response_delivers_and_returns_202() {
    ...
    let req = Request::post(format!("/2018-06-01/runtime/invocation/{}/response", req_id))
        .body(body)
        .unwrap();
...
async fn runtime_error_delivers_with_kind_and_returns_202() {
    ...
    let mut req = Request::post(format!("/2018-06-01/runtime/invocation/{}/error", req_id))
        ...
        .unwrap();
    req.headers_mut().insert("X-Amz-Function-Error", "Unhandled".parse().unwrap());

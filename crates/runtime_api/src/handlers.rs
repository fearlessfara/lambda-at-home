            ...
            .await
        {
            Ok(inv) => {
                ...
                let mut res = Response::new(Body::from(
                    serde_json::to_vec(&inv.payload).unwrap_or_default(),
                ));
                *res.status_mut() = StatusCode::OK;
                ...
                headers.insert(
                    ...
                    HeaderValue::from_str(&inv.aws_request_id.to_string()).unwrap(),
                );
                headers.insert(
                    HeaderName::from_static("lambda-runtime-deadline-ms"),
                    HeaderValue::from_str(&inv.deadline_ms.to_string()).unwrap(),
                );
                headers.insert(
                    HeaderName::from_static("lambda-runtime-invoked-function-arn"),
                    HeaderValue::from_str(&inv.invoked_function_arn).unwrap(),
                );
                if let Some(tid) = &inv.trace_id {
                    if let Ok(hv) = HeaderValue::from_str(tid) {
                        headers.insert(HeaderName::from_static("lambda-runtime-trace-id"), hv);
                    }
                }
                return res;
            }
        ...
        match state.queues.pop_or_wait(&key).await {
            Ok(work_item) => {
                ...
                *res.status_mut() = StatusCode::OK;
                ...
                headers.insert(
                    HeaderName::from_static("lambda-runtime-aws-request-id"),
                    HeaderValue::from_str(&work_item.request_id).unwrap(),
                );
                headers.insert(
                    HeaderName::from_static("lambda-runtime-deadline-ms"),
                    HeaderValue::from_str(&work_item.deadline_ms.to_string()).unwrap(),
                );
                headers.insert(
                    HeaderName::from_static("lambda-runtime-invoked-function-arn"),
...
pub async fn runtime_response(
    ...
    if let Some(control) = state.control.clone() {
        ...
        let rr = RuntimeResponse {
            aws_request_id: Uuid::try_parse(&request_id).unwrap_or_else(|_| Uuid::nil()),
        ...
        let hdrs = {
            ...
            {
                map.insert("X-Amz-Executed-Version".to_string(), v.to_string());
            }
            if let Some(v) = headers
                .get("X-Amz-Log-Result")
                .and_then(|h| h.to_str().ok())
            {
                map.insert("X-Amz-Log-Result".to_string(), v.to_string());
            }
            if map.is_empty() {
                None
            } else {
...
pub async fn runtime_error(
    ...
    if let Some(control) = state.control.clone() {
        ...
        let re = RuntimeError {
            aws_request_id: Uuid::try_parse(&request_id).unwrap_or_else(|_| Uuid::nil()),
            error_message: payload_json.to_string(),
            error_type: err_kind.clone(),
            stack_trace: None,
        };
        let hdrs = {
            let mut map = std::collections::HashMap::new();

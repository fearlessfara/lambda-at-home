...
impl ConcurrencyManager {
    ...
    pub async fn acquire_token(&self, function: &Function) -> Result<TokenGuard, LambdaError> {
        ...
        let per = { self.per_function.lock().unwrap().get(&function.function_id).cloned() };
        if let Some(c) = per {
    ...
    pub fn try_acquire_token(&self, function: &Function) -> Result<TokenGuard, LambdaError> {
        let per = { self.per_function.lock().unwrap().get(&function.function_id).cloned() };
        if let Some(c) = per {
            return c.sem.clone().try_acquire_owned().map(|permit| TokenGuard { _permit: permit })
                .map_err(|_| LambdaError::InternalError { reason: "No concurrency tokens available".to_string() });
        }
    ...
    pub fn set_reserved_limit(&self, function_id: uuid::Uuid, limit: Option<u32>) {
        ...
        match limit {
            Some(n) => { map.insert(function_id, Concurrency::new(n as usize)); },
            None => { map.remove(&function_id); },
        }
    }
}

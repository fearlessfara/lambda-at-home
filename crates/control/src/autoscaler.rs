...
use crate::{warm_pool::InstanceState, registry::ControlPlane};
...
impl Autoscaler {
    ...
    async fn reconcile_once(&self) -> anyhow::Result<()> {
        ...
        for (key, qsize) in sizes {
            if qsize == 0 { continue; }
            let idle = self.control.warm_pool().count_state(&key, InstanceState::WarmIdle).await;
            let stopped = self.control.warm_pool().count_state(&key, InstanceState::Stopped).await;
            let (to_restart, to_create) = plan_scale(qsize, idle, stopped);
            if to_restart == 0 && to_create == 0 { continue; }
            ...
            for cid in stopped_ids.into_iter().take(to_restart) {
                info!("autoscaler: restarting stopped container {} for {}", cid, key.function_name);
                if let Err(e) = self.control.invoker().start_container(&cid).await { error!("start failed: {}", e); continue; }
                let _ = self.control.warm_pool().set_state_by_container_id(&cid, InstanceState::WarmIdle).await;
            }

            // Create new containers as needed
            for _ in 0..to_create {
                if let Err(e) = self.create_one(&key).await { error!("create failed: {}", e); }
            }
        }
        Ok(())
    }
    ...
    async fn create_one(&self, key: &crate::queues::FnKey) -> anyhow::Result<()> {
        ...
        let container_id = self.control.invoker().create_container(&function, &image_ref, env_vars).await?;
        self.control.invoker().start_container(&container_id).await?;
        ...
        self.control.warm_pool().add_warm_container(key.clone(), wc).await;
        info!("autoscaler: created container {} for {}", container_id, key.function_name);
        Ok(())
    }
}
...
pub fn plan_scale(queue_size: usize, warm_idle: usize, stopped: usize) -> (usize, usize) {
    if queue_size <= warm_idle { return (0, 0); }

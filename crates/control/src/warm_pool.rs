...
#[derive(Clone)]
pub struct WarmPool {
    // Key by FnKey (function_name, runtime, version, env_hash) for proper isolation
...
impl WarmPool {
    ...
    pub async fn get_warm_container(&self, key: &FnKey) -> Option<WarmContainer> {
        ...
        if let Some(container_list) = containers.get_mut(key) {
            // Find first available container for this FnKey
    ...
    pub async fn add_warm_container(&self, key: FnKey, container: WarmContainer) {
        ...
        info!("Added warm container: {} for function: {} runtime: {} version: {} env_hash: {}", 
              container_id, key.function_name, key.runtime, key.version, key.env_hash);
    }
    ...
    /// Get the number of warm containers for a specific FnKey
    pub async fn container_count(&self, key: &FnKey) -> usize {
        let containers = self.containers.lock().await;
        containers.get(key).map(|list| list.len()).unwrap_or(0)
    }
    
    /// Get total number of warm containers across all FnKeys
